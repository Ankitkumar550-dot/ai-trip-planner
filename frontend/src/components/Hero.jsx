...
function Hero() {
  ...
  const onSend = (val) => {
    ...
    if (!user) {
      // For Clerk React, we handle redirect manually or use <SignInButton />
      // But here we can just navigate to a custom sign-in page if we had one, 
      // or just use the Clerk redirect.
      navigate(`/sign-in?redirect_url=/create-new-trip?input=${encodeURIComponent(finalInput)}`)
      return;
    }
    navigate(`/create-new-trip?input=${encodeURIComponent(finalInput)}`)
  }
      ...
      <div className="relative max-w-4xl w-full text-center space-y-8 z-10">
        ...
        <div className="mt-8 rounded-3xl overflow-hidden shadow-2xl">
          <HeroVideoDialog
            ...
            videoSrc="https://www.youtube.com/embed/dQw4w9WgXcQ" // Example video
