...
export const Timeline = ({ data, tripData }) => {
  ...
  return (
    <div
      className="w-full bg-white dark:bg-neutral-950 font-outfit md:px-10"
      ref={containerRef}
    >
      <div className="max-w-7xl mx-auto py-20 px-4 md:px-8 lg:px-10 text-left">
        ...
        <div className="flex gap-5 items-center mt-6">
          <div className="flex gap-2 items-center bg-gray-50 px-4 py-2 rounded-full border">
            <Wallet2 className="w-4 h-4 text-indigo-600" />
            <h2 className="text-sm font-medium">{tripData?.budget}</h2>
          </div>
          <div className="flex gap-2 items-center bg-gray-50 px-4 py-2 rounded-full border">
            <Calendar className="w-4 h-4 text-pink-500" />
            <h2 className="text-sm font-medium">{tripData?.duration}</h2>
          </div>
          <div className="flex gap-2 items-center bg-gray-50 px-4 py-2 rounded-full border">
            <Users className="w-4 h-4 text-orange-500" />
            <h2 className="text-sm font-medium">{tripData?.group_size}</h2>
          </div>
        </div>
      </div>
        ...
        {data.map((item, index) => (
          ...
          >
            <div className="sticky flex flex-col md:flex-row z-40 items-center top-40 self-start max-w-xs lg:max-w-sm md:w-[40%]">
              <div className="h-10 absolute left-3 md:left-3 w-10 rounded-full bg-white dark:bg-black flex items-center justify-center border shadow-sm">
